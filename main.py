...
def collide2(x1, y1, x2, y2, x3, y3, x4, y4, size):
    return (
        (x3 + (11 * size)) > x1 > x3 - 1
        and (y3 + (11 * size)) > y1 > y3 - 1
        or (x3 + (11 * size)) > x2 > x3 - 1
        and (y3 + (11 * size)) > y2 > y3 - 1
    )
...
def collide3(x1, y1, x2, y2, x3, y3, x4, y4, size):
    return (
        (x3 + (10 * size)) > x1 > x3
        and (y3 + (10 * size)) > y1 > y3
        or (x3 + (10 * size)) > x2 > x3
        and (y3 + (10 * size)) > y2 > y3
    )
...
class snake():
    ...
    def update(self):
        ...
        for a, x in enumerate(self.images, start=1):
            x[1] = self.old_pos[int(a * ((-11 * self.size) / self.speed))]

    def check_collisions(self, x):
        return collide(self.pos[0], self.pos[1], self.pos[0] + 10,
...
class apple():
    def __init__(self, size):
        self.pos = [random.randrange(10, 780, 10),
                    random.randrange(10, 430, 10)]
        self.image = pg.Surface((10 * size, 10 * size))
...
class game():
    ...
    def over(self):
        while 1:
            ...
            txts = pg.font.SysFont('Courier New', 70).render(
                'Score:', True, (255, 255, 255))
            txtrect = txts.get_rect()
            txtrect.topleft = (225, 150)
            self.screen.blit(txts, txtrect)
            txts = pg.font.SysFont('Courier New', 70).render(
                str(self.snake.score), True, (255, 255, 255))
            txtrect = txts.get_rect()
            txtrect.topleft = (475, 150)
            self.screen.blit(txts, txtrect)
            pg.display.update()
            self.make_button((300, 300, 125, 62), 'Restart', [(255, 255, 255), (150, 150, 150)],
                             action=lambda: restart())
            if self.hover == True:
                click = pg.mouse.get_pressed()
                if click[0] == 1:
                    self.click0 = True
                if self.click0 == True and click[0] == 0:
                    self.buttonclick()
                    self.click0 = False
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        txts = pg.font.SysFont('Courier New', textsize).render(
            text, True, (0, 0, 0))
        txtrect = txts.get_rect()
    ...
    def loop(self):
        ...
        while not self.game_over:
            ...
            for a, x in enumerate(self.snake.images):
                if a != 0 and self.snake.check_apple(x[1]) == True:
                    self.over()
                self.screen.blit(x[0], x[1])
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    if event.key == pg.K_RIGHT and self.left == False:
                        self.reset()
                        self.snake.right()
                        self.right = True
                    if event.key == pg.K_LEFT and self.right == False:
                        self.reset()
                        self.snake.left()
                        self.left = True
                    if event.key == pg.K_UP and self.down == False:
                        self.reset()
                        self.snake.up()
                        self.up = True
                    if event.key == pg.K_DOWN and self.up == False:
                        self.reset()
                        self.snake.down()
                        self.down = True
            pg.display.update()
...
class startmenu():
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        rectangle.fill((255, 255, 255, alpha), special_flags=pg.BLEND_RGBA_MIN)
        self.screen.blit(rectangle, pos)
        self.make_text((pos[0] + pos[2] / 2), (pos[1] +
                       pos[3] / 2), text, a=True, size=textsize)

    def mainloop(self):
        while 1:
            self.screen.fill((100,100,100))
            self.make_text(400, 150, 'SNAKE', color=(
                255, 255, 255), size=80, a=True)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    sys.exit()
            for x in self.blocks:
    ...
    def start(self):
        self.b1 = '(150, 300,100,50),"Normal", [(216,216,216), (150,150,150)], action = self.start3'
        self.b2 = '(550, 300,100,50),"Big", [(216,216,216), (150,150,150)], action = self.start4'
        self.buttons = [self.b1, self.b2]

    def start3(self):
        self.b2 = '(200, 300,100,50),"Normal", [(216,216,216), (150,150,150)], self.n'
        self.b3 = '(350, 300,100,50),"Hard", [(216,216,216), (150,150,150)], action = self.h'
        self.b4 = '(500, 300,100,50),"Expert", [(216,216,216), (150,150,150)], action = self.ex'
        self.buttons = [self.b2, self.b3, self.b4]
    ...
    def e(self):
        ...
        class snake():
            ...
            def update(self):
                ...
                self.pos[1] += self.direction[1]
                for a, x in enumerate(self.images, start=1):
                    x[1] = self.old_pos[int(
                        a * ((-11 * self.size) / self.speed))]
        ...
        class apple():
            def __init__(self, size):
                self.pos = [random.randrange(
                    10, 780, 10), random.randrange(10, 430, 10)]
                self.image = pg.Surface((10 * size, 10 * size))
                self.image.fill((255, 0, 0))
            ...
            def over(self):
                while 1:
                    ...
                    txts = pg.font.SysFont('Courier New', 50).render(
                        'Game Over Score:', True, (255, 255, 255))
                    txtrect = txts.get_rect()
                    txtrect.topleft = (100, 150)
                    self.screen.blit(txts, txtrect)
                    txts = pg.font.SysFont('Courier New', 50).render(
                        str(self.snake.score), True, (255, 255, 255))
                    txtrect = txts.get_rect()
                    ...
                    self.screen.blit(txts, txtrect)
                    pg.display.update()
                    self.make_button((300, 300, 100, 50), 'Restart', [(255, 255, 255), (150, 150, 150)],
                                     action=lambda: restart())
                    if self.hover == True:
                        click = pg.mouse.get_pressed()
                        if click[0] == 1:
        ...
        class startmenu():
            def __init__(self):
                self.screen = pg.display.set_mode((800, 450))
                self.b1 = '(150, 300,100,50),"Start", [(216,216,216), (216,216,216)], action = self.start'
                self.b2 = '(550, 300,100,50),"Exit", [(216,216,216), (216,216,216)], action = self.exit'
                self.buttons = [self.b1, self.b2]
                self.blocks = []
                self.size = 1
                self.click0, self.loads = False, False
            ...
            def make_text(self, x, y, text, size=20, color=(0, 0, 0), a=False):
                txts = pg.font.SysFont(
                    'Courier New', size).render(text, True, color)
                txtrect = txts.get_rect()
                txtrect.topleft = (x, y)
                if a == True:
                    txtrect.center = (x, y)
            ...
            def make_button(self, pos, text, color, action=None, textsize=20):
                ...
                self.screen.blit(rectangle, pos)
                self.make_text(
                    (pos[0] + pos[2] / 2), (pos[1] + pos[3] / 2), text, a=True, size=textsize)
            ...
            def start(self):
                self.b1 = '(150, 300,100,50),"Normal", [(216,216,216), (216,216,216)], action = self.start3'
                self.b2 = '(550, 300,100,50),"Big", [(216,216,216), (216,216,216)], action = self.start4'
                self.buttons = [self.b1, self.b2]

            def start3(self):
                self.b1 = '(150, 300,100,50),"Easy", [(216,216,216), (216,216,216)], action = self.e'
                self.b2 = '(283, 300,100,50),"Normal", [(216,216,216), (216,216,216)], self.n'
                self.b3 = '(417, 300,100,50),"Hard", [(216,216,216), (216,216,216)], action = self.h'
                self.b4 = '(550, 300,100,50),"Expert", [(216,216,216), (216,216,216)], action = self.ex'
